...
BOOL tryElevate(BOOL silent)
{
    ...
    {
        if (!silent)
            LOG("Unsupported Windows version. clumsy only supports Windows Vista or above.\nAborting");
        return TRUE;
    }
    ...
    if (!silent)
    {
        ...
        if (GetModuleFileName(NULL, (LPSTR)szPath, ARRAYSIZE(szPath)))
        {
            ...
            if (!ShellExecuteEx(&sei))
            {
                ...
                if (dwError == ERROR_CANCELLED)
                {
                    ...
                    // alert and exit
                    LOG("clumsy needs to be elevated to work. Run as Administrator or click Yes in promoted UAC dialog.\nAborting");
                }
            }
            else
            {
        ...
        {
            LOG("Failed to get clumsy path. Please place the executable in a normal directory.\nAborting");
        }
    }
